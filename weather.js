(function() {
    ...
    var lastWeatherUpdate = localStorage.getItem('lastWeatherUpdate');
    var weather = JSON.parse(localStorage.getItem('weather'));
    
    setWeatherDiv(weather.temp);

    var d = new Date();
    var timeNow = d.getHours();

    if ((timeNow - lastWeatherUpdate) !== 0) {
        console.log('Weather is updating...');
        localStorage.setItem('lastWeatherUpdate', timeNow);
